...
const chapter=cheerio.load(fs.readFileSync(path.join(output,'wiki/networks/physical-layer/index.html'),'utf8'));
assert(chapter('.katex').length>=9,'Math did not render');
...
assert(chapter('a[href="/wiki/networks/network-overview/"]').length,'Previous chapter missing');
assert(chapter('a[href="/wiki/networks/data-link-crc/"]').length,'Next chapter missing');
...
console.log(`PASS: ${pages} pages, local links, linked thoughts, page banners, footer, visitor counters, local LXGW WenKai, 9 math expressions, TOC, chapter navigation, search index, importer and overwrite protection.`);
